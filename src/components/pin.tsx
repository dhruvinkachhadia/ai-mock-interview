...
export const InterviewPin = ({
  ...
  // ✅ Handle missing `createdAt` to prevent `.toDate()` errors
  let formattedDate = "No date available";
  if (interview?.createdAt) {
    try {
      const dateObj = interview.createdAt.toDate();
      formattedDate = `${dateObj.toLocaleDateString("en-US", { dateStyle: "long" })} - ${dateObj.toLocaleTimeString("en-US", { timeStyle: "short" })}`;
    } catch (error) {
